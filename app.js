...
function renderKPIs(){
  ...
  // NEW metrics
  $('#emailsVal').textContent = (last.emails || 0).toLocaleString();
  $('#ugcVal').textContent = ugc.length.toLocaleString();
  ...
  $('#emailsTarget').textContent = targets.emails.toLocaleString();
}
...
function updateEditableState(){
  ...
  // Admin: Full access
  if (role === 'admin') {
    $all('.editable').forEach(el => el.style.display = '');
    $all('input, select, textarea, button').forEach(el => el.disabled = false);
  }

  // Contributor: Partial access (edit content but not settings/export)
  else if (role === 'contributor') {
    $all('.editable').forEach(el => el.style.display = '');
    $all('input, select, textarea, button').forEach(el => el.disabled = false);
    $('#exportCsvBtn').disabled = true; // restrict CSV export
  }

  // Officer: View-only access
  else if (role === 'officer') {
    $all('.editable').forEach(el => el.style.display = 'none');
    $all('input, select, textarea, button').forEach(el => {
      if (!el.closest('.role-label')) el.disabled = true; // keep role dropdown enabled
    });
  }
}
